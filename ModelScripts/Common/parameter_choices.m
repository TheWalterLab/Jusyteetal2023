...
%Model 47: par_free = [Q_max, Ca_prim_type, prim_kM, prim_rate_const, unprim_rate_const] % Abel : m
model_type_vec(q) = 47; num_pars_vec(q) = 8; q = q+1;
...
if sum(par_free ~= 0)
    ...
    elseif model_type == 47
        ...
        Q_max = par_free(1);
        Ca_prim_type = par_free(2);
        prim_kM = par_free(3); % Abel : m
        prim_rate_const = par_free(4);
        unprim_rate_const = par_free(5);
        CaMax_rest = par_free(6); %Aymptotic value of resting calcium (with increasing CaExt) Abel: changed it from 190nM to 300nM
        num_ves = round(par_free(7)*180);
        unprim_mutant = par_free(8);
